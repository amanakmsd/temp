...
class Doctor extends CI_Controller {
    ...
    public function registration_number($registration_number, $user_id)
    {
        $licenceNoExists = $this->db->select('user_id')
        ...
        if ($licenceNoExists > 0) {
            $this->form_validation->set_message('registration_number', 'The {field} field must contain a unique value.');
            return false;
        } else {
            return true;
        }
	...
	public function create()
	{
		...
		if ($this->input->post('user_id',true) == null) {
			...
			$this->form_validation->set_rules('registration_number', 'Registration Number', "required|max_length[50]|callback_registration_number[$cur_user_id]");
		}
    ...
    public function doctor_schedule_by_date()
    {
        ...
        $doctor_id = $this->my_encrypt->decode($doctor_id);
