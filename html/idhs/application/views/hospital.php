                            ...
                            <?php foreach ($hospitals as $hospital) { ?>
                                <tr class="<?php echo ($sl & 1)?"odd gradeX":"even gradeC" ?>">
                                    ...
                                    <td>
                                        ...
                                        <a href="<?php echo base_url("hospital/approve/".$this->my_encrypt->encode($hospital->user_id)) ?>" class="btn btn-xs btn-danger" onclick="return confirm('Are you sure! you want to approve?')"><i class="fa fa-check"></i></a>
                                        <?php } ?>
                                        <a href="<?php echo base_url("hospital/profile/".$this->my_encrypt->encode($hospital->hospital_id)) ?>" class="btn btn-xs btn-success"><i class="fa fa-eye"></i></a> 
                                        <a href="<?php echo base_url("hospital/edit/".$this->my_encrypt->encode($hospital->hospital_id)) ?>" class="btn btn-xs btn-primary"><i class="fa fa-edit"></i></a> 
                                        <a href="<?php echo base_url("hospital/delete/".$this->my_encrypt->encode($hospital->hospital_id)) ?>" class="btn btn-xs btn-danger" onclick="return confirm('Are you sure! You want to delete? ')"><i class="fa fa-trash"></i></a>
                                        </div> 
                                    </td>
                                </tr>
                                <?php $sl++; ?>
