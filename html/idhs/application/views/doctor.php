                            ...
                            <?php foreach ($doctors as $doctor) { ?>
                                <tr class="<?php echo ($sl & 1)?"odd gradeX":"even gradeC" ?>">
                                    ...
                                    <td>
                                        ...
                                        <a href="<?php echo base_url("doctor/profile/".$this->my_encrypt->encode($doctor->user_id)) ?>" class="btn btn-xs btn-success"><i class="fa fa-eye"></i></a>
                                        <?php } ?>
                                        <?php if($this->session->userdata('user_role') == 2 || in_array('2', $user_permissions)) { ?>
                                        <a href="<?php echo base_url("doctor/edit/".$this->my_encrypt->encode($doctor->user_id)) ?>" class="btn btn-xs btn-primary"><i class="fa fa-edit"></i></a>
                                        <a href="<?php echo base_url("doctor/delete/".$this->my_encrypt->encode($doctor->user_id)) ?>" class="btn btn-xs btn-danger" onclick="return confirm('Are you sure! You want to delete?')"><i class="fa fa-trash"></i></a>
                                        <?php } ?>
                                        </div> 
                                    </td>
