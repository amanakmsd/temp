...
class Dashboard_model extends CI_Model {
	...
	{
		return $this->db->select("user.*, hospital.picture hospital_logo, hospital.name hospital_name, hospital.organs_operated, hospital.disease_operated, hospital.hospital_avaiability")
			->from($this->table)
    ...
    public function insert_hospital($data = [])
	{
		...
		$user_array = array('email' => strtolower($data['email']), 'password' => $data['password'], 'user_role' => $data['user_role'], 'create_date' => $data['create_date'], 'update_date' => $data['update_date'], 'status' => 2);
		$this->db->insert('user',$user_array);
		$user_id = $this->db->insert_id();

		$toRemove = ['email', 'password', 'user_role', 'status', 'signup_otp']; 
	...
	public function verify_otp($data = [])
	{
		return $this->db->select("*")
			->from("hospital_temp")
			->where('hospital_id', $data['user_id'])
			->where('signup_otp',$data['signup_otp'])
			->where('status', 0)
			->get()
			->row_array();
	}

	public function check_temp_hospital($data = [])
	{
		return $this->db->select("*")
			->from("hospital_temp")
			->where('email',$data['email'])
	...
	public function insert_temp_hospital($data = [])
	{
		$this->db->insert('hospital_temp',$data);
		return $this->db->insert_id();
	}

  	public function update_temp_hospital($data = [])
	{
		return $this->db->where('hospital_id',$data['hospital_id'])
			->update("hospital_temp" ,$data); 
	} 
}
