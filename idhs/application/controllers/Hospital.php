...
class Hospital extends CI_Controller {
		...
		if ($this->form_validation->run() === true) {
			...
			if (empty($postData['hospital_id'])) {
				if ($this->hospital_model->create($postData)) {
					#set success message
					$this->session->set_flashdata('message', 'Hospital registration has been completed successfully.');
				} else {
				...
				/*if ($postData['hospital_id'] == $this->session->userdata('user_id')) {
				...
				redirect('hospital/edit/'.$postData['hospital_id']);
			}
	...
	public function approve($user_id = null) 
	{
		...
		if ($this->hospital_model->approve($user_id)) {
			#set success message
